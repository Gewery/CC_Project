...
int yylex(void) {
if(!yyin) yyin = stdin; if(seeneof) return 0;
while(1) {
int c = getc(yyin);
if(isdigit(c)) { int i = c - '0';
/* saw EOF last time */
while(isdigit(c = getc(yyin))) i = (10*i) + c-'0';
yylval = i;
if(c == EOF) seeneof = 1; else ungetc(c, yyin); return NUMBER;
}
switch(c) {
...
break; }
if(c == EOF) seeneof = 1; /* it's division */ else ungetc(c, yyin);
return DIV;
