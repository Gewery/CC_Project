...
if (!variabledeclarations->variabledeclarations) print_VariableDeclaration(variabledeclarations->variabledeclaration, 1);
...
add_spaces_to_prefix(2 + 9);
if (!statement->routinecall && !statement->whileloop && !statement->forloop && !statement->ifstatement) print_Assignment(statement->assignment, 1);
else print_Assignment(statement->assignment, 0);
if (!statement->whileloop && !statement->forloop && !statement->ifstatement) print_RoutineCall(statement->routinecall, 1);
...
if (!multiplerelationsinexpression->relation && !multiplerelationsinexpression->multiplerelationsinexpression) print_LogicalOperator(multiplerelationsinexpression->logicaloperator, 1);
else print_LogicalOperator(multiplerelationsinexpression->logicaloperator, 0);
if (!multiplerelationsinexpression->multiplerelationsinexpression) print_Relation(multiplerelationsinexpression->relation, 1);
